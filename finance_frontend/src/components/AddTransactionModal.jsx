...
import { X, Plus, Calendar, MessageSquare, Tag } from 'lucide-react';
import { useLanguage } from '../context/LanguageContext';
import { financesAPI } from '../services/api';
import toast from 'react-hot-toast';

export default function AddTransactionModal({ isOpen, onClose, type = 'expense', onSuccess }) {
  const { t } = useLanguage();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (type === 'income') {
        await financesAPI.createIncome(data);
        toast.success(t('income.incomeAdded'));
      } else {
        await financesAPI.createExpense(data);
          ...
          <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }} className="relative w-full max-w-md glass-card p-6">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-display font-bold text-white">
                {type === 'income' ? t('income.addIncome') : t('expenses.addExpense')}
              </h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button type="submit" disabled={loading} className={`flex-1 btn-primary flex items-center justify-center gap-2 ${type === 'income' ? 'from-income-600 to-income-500' : ''}`}>
                  {loading ? <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : <><Plus className="w-5 h-5" /><span>{t('common.add')}</span></>}
                </button>
              </div>
            </form>
          </motion.div>
